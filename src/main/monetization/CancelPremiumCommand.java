...
public final class CancelPremiumCommand extends Command {
    private static final int ONE_MILLION = 1000000;
    public CancelPremiumCommand(final Command command) {
    ...
    public void execCommand() {
        ...
        int totalListensInTimeframe = UserSpaceDb.getDatabase().get(getUsername())
                .getTop().getTimeFrameListenedSongs().size();
        HashMap<String, Integer> songsPerArtist = new HashMap<>();

        int total = 0;
        for (Map.Entry<String, Integer> listenedSong : UserSpaceDb.getDatabase()
                .get(getUsername()).getTop().getTimeFrameListenedSongs().entrySet()) {
        ...
        for (Map.Entry<String, Integer> listenedSong : UserSpaceDb.getDatabase()
            ...
            if (artist != null) {
                ...
                double moneyPerSong = (double) ONE_MILLION * listenedSong.getValue() / total;
        ...
        for (Map.Entry<String, Integer> listens : songsPerArtist.entrySet()) {
            ...
            double moneyPerSong = (double) ONE_MILLION * timesListenedArtist / total;
