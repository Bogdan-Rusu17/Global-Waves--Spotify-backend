...
public class BuyPremiumCommand extends Command {
    public BuyPremiumCommand(Command command) {
    ...
    public void execCommand() {
        ...
        if (UserSpaceDb.getDatabase().get(getUsername()).isPremiumUser()) {
            this.getObjectNode().put("message", getUsername() + " is already a premium user.");
            GlobalObjects.getInstance().getOutputs().add(this.getObjectNode());
            return;
        }
        UserSpaceDb.getDatabase().get(getUsername()).setPremiumUser(true);
        this.getObjectNode().put("message", getUsername() + " bought the subscription successfully.");
        GlobalObjects.getInstance().getOutputs().add(this.getObjectNode());
    }
}
